...
- (void)viewWillAppear:(BOOL)animated {
    ...
    CGRect frame = CGRectMake(0, 0, SCREEM_WIDTH, SCREEM_HEIGHT);
    
    self.scanningView = [[SGScanningQRCodeView alloc] initWithFrame:frame outsideViewLayer:self.view.layer];
...
- (void)leftBar {
    ...
    [leftButton setImage:[UIImage imageNamed:@"btn_back_w"] forState:UIControlStateNormal];
    
    [leftButton addTarget:self action:@selector(naviBtnAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)scanQRCodeFromPhotosInTheAlbum:(UIImage *)image {
    ...
    for (int index = 0; index < [features count]; index ++) {
        ...
        if (self.first_push) {
            ...
            if(_scannedResultBlock != nil){
                _scannedResultBlock(scannedResult);
            }
...
- (void)setupScanningQRCode {
    ...
    // 2、创建输入流
    AVCaptureDeviceInput *input = [AVCaptureDeviceInput deviceInputWithDevice:device error:&outError];
